...
TdmsMetaData::TdmsMetaData(TdmsParser *parser, bool verbose)
    : d_verbose(verbose), d_objectCount(0), d_channel_cnt(0), d_chunks(0),
      d_parser(parser) {
...
void TdmsMetaData::readObject(std::iendianfstream &file, const bool verbose) {
    ...
    if (o->isGroup()) {
        ...
        /* std::cout << channelName << ":" << groupName << std::endl; */

        // TODO: THIS HAPPENS ALWAYS - looking for group by name!
        ...
        if (channel == 0) {
            ...
            channel->setTypeSize((type == TdmsChannel::tdsTypeString)
                                     ? (unsigned int)o->getBytesCount()
...
unsigned int TdmsMetaData::getChannelCount() {
    if (d_channel_cnt == 0) {
        for (TdmsObjectList::iterator object = d_objects.begin();
             object != d_objects.end(); ++object) {
...
unsigned int TdmsMetaData::getInterleavedChannelCount(TdmsObject *obj,
                                                      TdmsChannel *channel) {
    if (!channel || !obj)
        return 0;
    ...
    std::vector<FormatChangingScaler> formatScalersVector =
        obj->formatScalersVector();
    if (formatScalersVector.empty())
        return 0;

    unsigned int cnt = 0,
                 rawBufferIndex = formatScalersVector.front().rawBufferIndex;
        ...
        if (ch) {
            for (TdmsObjectList::iterator object = d_objects.begin();
                ...
                if (!o)
                    continue;
                if (o->getChannel() == ch) {
                    std::vector<FormatChangingScaler> fsv =
                        o->formatScalersVector();
                    if (fsv.empty())
                        return 0;
...
TdmsObject *TdmsMetaData::readRawData(unsigned long long total_chunk_size,
                                      TdmsObject *prevObject) {
    if (!total_chunk_size)
        return 0;
    ...
    if (interleavedData) {
        for (TdmsObjectList::iterator object = d_objects.begin();
            ...
            if (channel)
                channel->setInterleavedFlag(
                    interleavedData &&
                    getInterleavedChannelCount(obj, channel));
        }
    }
        ...
        for (TdmsObjectList::iterator object = d_objects.begin();
            ...
            if (obj->hasDAQmxData()) {
                ...
                if (!channel)
                    channel = getChannel(obj);
                if (channel) {
                    d_chunk_raw_data_pos.push_back(
                        (unsigned long long)file.tellg());
...
TdmsObject *TdmsMetaData::readChannelRawData(TdmsChannel *channel,
                                             TdmsSegment *seg,
                                             TdmsObject *prevObject) {
    ...
    if (interleavedData && obj->hasDAQmxData())
        channel->setInterleavedFlag(interleavedData &&
                                    getInterleavedChannelCount(obj, channel));
